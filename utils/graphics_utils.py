...
class BasicPointCloud(NamedTuple):
    ...
    time : np.array = None
    obj_id : np.array = None

def get_bound_diagonal_distance(pcd):
    bound_max = np.max(pcd, axis=0)
    bound_min = np.min(pcd, axis=0)
    bound_size = np.sqrt(np.sum((bound_max - bound_min) ** 2))
    return bound_size
...
def theta_to_vector(x):
    return torch.cat([
        torch.cos(x[..., :1]) * torch.cos(x[..., 1:]),
        torch.sin(x[..., :1]) * torch.cos(x[..., 1:]),
        torch.sin(x[..., 1:]),
    ], dim=-1)

def vector_to_theta(x):
